	...
	function get_ref($event)
	{
		if ($this->user->page['script_path'] != '/adm/')
		{
			...
			if (isset($data['module']) && $data['module'] !== '')
			{
				if (($this->ip_cache = $this->cache->get('_ip_cache')) !== false)
				{
					if (!isset($this->ip_cache[$this->user->data['session_ip']]))
					{
						...
						} else
						{
							...
							$this->ip_cache[$this->user->data['session_ip']]['countryCode'] = ($this->user->data['session_ip'] == '127.0.0.1') ? 'loc' : strtolower($aray[sizeof($aray) -1]);
						}
					}
				} else
				{
				...
				$data['domain'] = (!file_exists('./ext/forumhulp/statistics//adm/style/images/flags/' . $data['domain'] . '.png')) ? 'unk' : $data['domain'];
				...
				{
					$data['screen_res'] = $this->request->variable($this->config['cookie_name'] . '_statistics_res', 'unknown', false, \phpbb\request\request_interface::COOKIE);
				}
